...
export function StepConfirmation({
  ...
  const handleSubmitBooking = async () => {
    ...
    // Simula envio para API / Server Action
    await new Promise((resolve) => setTimeout(resolve, 1200));

    const generatedProtocol = `VB-2026-${Math.floor(1000 + Math.random() * 9000)}`;
    ...
    <div className="flex flex-col gap-[2rem]">
      <div>
        ...
        <p className="text-[#444840] text-[0.9375rem] mt-[0.25rem]">
          Confira se todas as informações estão corretas antes de enviar a
          solicitação de agendamento para a profissional.
        </p>
      </div>
          ...
          <div className="flex flex-col gap-[0.75rem] text-[0.875rem]">
            <div>
              ...
              <p className="text-[#444840] text-[0.8125rem]">
                Duração estimada: {service?.duration} • Valor: {service?.price}
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-[#7d7a75] text-[0.8125rem]">
                {professional?.role} ({professional?.crnOrCoren})
              </p>
            </div>
